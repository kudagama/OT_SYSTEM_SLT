...
/** Compact shift label e.g. "8AM–4PM" or "1st Off" */
...
export default function WeeklySchedule() {
  ...
  // schedule = { "YYYY-MM-DD": "shiftType", ... } — sourced from DB
  const [schedule, setSchedule]   = useState({});
  const [fetching, setFetching]   = useState(true);
  const [pickerDay, setPickerDay] = useState(null);
  const [saving, setSaving]       = useState(false);   // tracks in-flight API call
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted) setSchedule(res.entries || {});
      } catch {
        // Fallback: keep empty schedule (don't crash)
      } finally {
  ...
  // ── Set shift (PUT to DB) ─────────────────────────────────────────────────
  const setShift = useCallback(async (dateKey, shiftType) => {
    // Optimistic update
    setSchedule((prev) => ({ ...prev, [dateKey]: shiftType }));
    setPickerDay(null);
    setSaving(true);
    try {
  ...
  // ── Clear shift (DELETE from DB) ──────────────────────────────────────────
  const clearShift = useCallback(async (dateKey) => {
    // Optimistic update
    setSchedule((prev) => {
      const next = { ...prev };
      delete next[dateKey];
    ...
    try {
      ...
      // Revert not straightforward — just refetch
      try {
        const res = await api.getSchedule();
        setSchedule(res.entries || {});
      } catch { /* ignore */ }
      ...
      {fetching ? (
        <div className="flex gap-2">
          {Array.from({ length: 7 }, (_, i) => (
            <div key={i} className="flex-1 h-[72px] rounded-xl bg-dark-700 animate-pulse" />
          ))}
        </div>
      ) : (
